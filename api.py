...
if FAP_ENV == "production" and not FAP_API_KEY: raise ValueError("CRITICAL: FAP_API_KEY environment variable must be set in production")
...
@app.get("/")
async def root():
    return HTMLResponse("<h1>FAP-Core</h1><p>Fraud-resistant provenance verification for insurance claims.</p>")

@app.exception_handler(RateLimitExceeded)
async def rl_handler(request, exc): return JSONResponse(status_code=429, content={"detail": "Rate limit exceeded"})

def verify_key(credentials: HTTPAuthorizationCredentials = Depends(security)):
    if not credentials: raise HTTPException(status_code=403, detail="Missing API key. Provide via Authorization: Bearer <key>")
    if FAP_API_KEY and credentials.credentials == FAP_API_KEY: return credentials.credentials
...
def _decode_epm_exchange(value: str) -> dict:
    try: data = json.loads(base64.urlsafe_b64decode(value + "=" * (-len(value) % 4)).decode("utf-8"))
    except (ValueError, TypeError, UnicodeDecodeError, json.JSONDecodeError) as exc: raise HTTPException(status_code=400, detail="Invalid EPM assurance envelope.") from exc
...
async def verify(request: Request, req: VerifyRequest, api_key: str = Depends(verify_key)):
    ...
    if epm_header:
        ...
        try: result.epm_attestation = build_attestation(exchange=exchange, verification=req.model_dump(mode="python", exclude_none=False), result=result.model_dump(mode="python"))
        except (ValueError, RuntimeError) as exc: raise HTTPException(status_code=400, detail="EPM assurance binding failed.") from exc
    return result
