...
def _normalize(value: Any) -> Any:
    ...
    if isinstance(value, (set, frozenset)): raise ValueError("unordered collections are not canonical")
    if isinstance(value, float) and (value != value or value in (float("inf"), float("-inf"))): raise ValueError("non-finite numbers are not canonical")
...
def _private_key() -> Ed25519PrivateKey:
    ...
    if not encoded: raise RuntimeError("FAP_EPM_ATTESTATION_PRIVATE_KEY is not configured")
    try: raw = base64.urlsafe_b64decode(encoded + "=" * (-len(encoded) % 4))
    except (ValueError, TypeError) as exc: raise RuntimeError("FAP_EPM_ATTESTATION_PRIVATE_KEY is invalid") from exc
...
def build_attestation(*, exchange: Mapping[str, Any], verification: Mapping[str, Any], result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    unsigned = {"exchange_version": EXCHANGE_VERSION, "request_id": exchange["request_id"], "nonce": exchange["nonce"], "request_digest": exchange["request_digest"], "evidence_id": exchange["evidence_id"], "artifact_id": result["artifact_id"], "engine_id": responder, "engine_version": engine_version, "policy_id": policy_id, "policy_version": policy_version, "oracle_versions": ["fap-core:internal-v1"], "processed_at": processed_at, "result": result["verdict"], "confidence": result["confidence"], "failure_state": None, "responder_service_id": responder}
